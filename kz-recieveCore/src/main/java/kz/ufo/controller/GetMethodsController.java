...
import kz.ufo.service.GetMethods;
...
public class GetMethodsController {
    ...
    public GetProviderByPhoneDTO getProvNameByPhone(@RequestBody PhoneProviderDTO phone){

        return getProviderList.getProviderNameByPhone(phone);
    }

    @GetMapping(value = "/getLogo" , produces = MediaType.APPLICATION_JSON_VALUE)
    ...
    public ListGetStatusPayDTO getStatusPay(@RequestBody ListStatusDTO statusIdDTO){
        ListGetStatusPayDTO   status = getProviderList.getStatusPay(statusIdDTO);
        return  status;
    }
    ...
    @GetMapping(value = "/getResultCodes" , produces = MediaType.APPLICATION_JSON_VALUE)
