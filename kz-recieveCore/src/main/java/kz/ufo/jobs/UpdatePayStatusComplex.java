...
@Slf4j
public class UpdatePayStatusComplex implements Job {
    @Autowired
    ...
    public void execute(JobExecutionContext jobExecutionContext) throws JobExecutionException {
        try {
            TblScheduleTime scheduleTime = tblScheduleTimeRepository.findByName("updatePayStatusComplex");
            if(scheduleTime.getEnabled()==1) {
                coreLogicMethods.checkAndUpdateStatusComplex();
            }
        }catch (Exception e){
            log.error("Error in checkAndUpdateStatusComplex",e);
